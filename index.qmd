...
## Step 4: Geographical Potential Results 
...
# Visualize
ggplot(suitability_summary, aes(x = class, y = area_km2, fill = as.character(class))) + 
  geom_col() +
...
rd <- sf::st_read("~/Library/CloudStorage/OneDrive-UBC/RA/Data/Admin Boundaries/BC_RegionalDistricts.shp")
...
ggplot(rd_Zstats_df, aes(fill = class, y = Area_km2, x = district_name)) +
...
```{r}
#| echo: false
#| fig-cap: Suitability within BC's Regional District, ranked in ascending order by amount of Class 4 and Class 5 land. No log scale is used.
...
ggplot(rd_Zstats_df, aes(fill = class, y = Area_km2, x = district_name)) +
  geom_bar(position = "stack", stat = "identity") +
  ggtitle("Suitability Class by Regional District") +
  scale_fill_manual(
    values = class_colors, 
    name = "Suitability Class",
    labels = c("1" = "Class 1 - Least Suitable",
               "2" = "Class 2 - Less Suitable",
...
writeRaster(pvout_mask, "/Users/kenziethomson/Library/CloudStorage/OneDrive-UBC/RA/SolarModel_Guide/data/energy/pvout_clean.tif")
```
...
Now, use a GIS GUI (ArcGIS Pro, QGIS) to transform each raster into a polygon layer - terra:as.polygons() never runs, thats why a seperate software is necessary. Once done, we will filter out sites smaller than 5 acres (minimum size threshold for a 1 MW installation)
...
## Step 4: Technical Potential Results
...
energy_rd <- sf::st_join(top_energy, bc, largest = TRUE)
