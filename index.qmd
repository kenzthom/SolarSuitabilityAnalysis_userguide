...
ggplot(suitability_summary, aes(x = factor(class), y = area_km2, fill = as.character(class))) + 
  geom_col() +
  geom_text(aes(label = paste0(round(percent_of_total, 2), "%")), 
            vjust = -0.5, size = 3) +
...
# Verify total percentage sums to 100
sum(core_suitability_summary$percent_of_total)
```

### 4.2 External Participants/Experts 
...
# Verify total percentage sums to 100
sum(external_suitability_summary$percent_of_total)
```

### 4.3 Sensitivity Analysis Comparison 
...
weights_comparison <- weights_comparison %>%
  ...
  mutate(Criteria = reorder(Criteria, Final_Weight, FUN = mean))
```

Visualize difference in weighting schemes
...
cat("The total number of sites are:", nrow(base_poly))
```

Filter by scenario and apply specific minimum map unit (mmu) filters
...
## Step 2: Create Custom Solar Resource Function
...
process_solar_scenario <- function(sf_layer, pvout_mask) {
  sf_layer %>%
    ...
    # Calculate annual energy (TWh)
    mutate(energy_TWh = Ei_kWh / 1e9)
}
...
sf::st_write(scen_100mw_outside_energy,"~/Library/CloudStorage/OneDrive-UBC/RA/SolarModel_Guide/data/energy/top_energy_above100MW_outsideMuni.shp", append = FALSE)
```
...
get_summary(scen_100mw_outside_energy, "Utility Generation (100 MW Outside Muni / 500-acre MMU)")
```
...
# Create summary for 1 MW (baseline study) that splits class 4 and class 5
...
get_summary(scen_1mw_energy_cl5, "Regular (1 MW / 5-acre MMU) Class 5")
```

Determine energy yield per regional district for 1MW installations. 
...
ggplot(RegDis_energy_df, aes(x = reorder(Reg_Dis, total_energy_TWh), y = total_energy_TWh)) +
  ...
  coord_flip() +
  labs(title = "Technical Potential by Regional District",
       x = "Regional District",
       y = "Annual Energy (TWh)") +
  theme_minimal() +
...
tn_rd <- sf::st_read("~/Library/CloudStorage/OneDrive-UBC/RA/SolarModel_Guide/data/admin boundaries/RegionalDistrict_Final.shp") %>%
...
topsis_results <- tibble(
  ...
  left_join(tn_sites %>% 
              select(site_id, Area_km2, cap_GW, energy_TWh),
            by = "site_id") %>%
  arrange(rank)
...
# Save
sf::st_write(tn_sites_ranked, "/Users/kenziethomson/Library/CloudStorage/OneDrive-UBC/RA/SolarModel_Guide/data/topsis/TN_topsis_rankings.shp", append = FALSE)
```
...
tn_sites_ranked <- sf::st_read("/Users/kenziethomson/Library/CloudStorage/OneDrive-UBC/RA/SolarModel_Guide/data/topsis/TN_topsis_rankings.shp", quiet = TRUE)
